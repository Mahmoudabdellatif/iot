...
$(document).ready(function () {
  ...
  if (
    ...
    alert(
      "This does not look like a mobile device - please load this page on a smartphone or tablet!"
    );
});
...
function smooth() {
  var heartSignal = AudioProcessor.savitzkyGolay(
    accelData,
    Math.round(fps / 2) * 2 + 1,
    3
  );
  heartSignal = AudioProcessor.savitzkyGolay(
    heartSignal,
    Math.round(fps / 4) * 2 + 1,
    3
  );
  var breathSignal = AudioProcessor.savitzkyGolay(
    heartSignal,
    Math.round(fps) * 2 + 1,
    3
  );
...
function calculateHeartRate(event) {
  ...
  if (new Date() - startTime > mduration) {
    ...
    if (averageBpm != 0) {
      ...
      $("#output").html(
        '<img src="/img/heart.png" height="77" width="77" align="center"><br/>Waiting for accelerometer to pick up your heartbeat... <b>Don\'t move</b>, keep the phone against your chest, and please be patient.<!--<h4><u>Firmly</u> press the bottom of your smartphone against your chest, and wait <u>without moving</u></h4>-->'
      );
    } else {
      $("#output").html(
        "<h1>This is not a mobile device, or no accelerometers detected. Please load this page on a smartphone or tablet instead!</h1>"
      );
    }
    ...
    if (
      bpm != 0 &&
      (averageBpm != 0 || (currentBpm > 50 && currentBpm < 140))
    ) {
      averageBpm =
        0.3 * currentBpm + 0.7 * (averageBpm + (currentBpm - averageBpm) / n);
      n++;
    }
    if (currentBpm < 50) currentBpm = 50;
    else if (currentBpm > 140) currentBpm = 140;
    bpm = currentBpm;
...
function clearData() {
  ...
  xhr.open(
    "get",
    "/unregister?type=" + iotfData.deviceType + "&id=" + iotfData.deviceId
  );
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
...
function onConnectionLost(response) {
  connStatus.innerHTML = '<span style="color:red"><b>Disconnected</b></span>';
...
function deviceOrientationHandler(tiltLR, tiltFB, dir) {
  ...
  document.getElementById("motion1").innerHTML = Math.round(tiltLR);
  document.getElementById("motion2").innerHTML = Math.round(tiltFB);
  document.getElementById("motion3").innerHTML = Math.round(dir);
}
...
function deviceMotionHandler(eventData) {
  ...
  var speed = Math.sqrt(
    parseInt(accelX) ^ (2 + parseInt(accelY)) ^ (2 + parseInt(accelZ)) ^ 2
  );
